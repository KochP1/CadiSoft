...
const getSecciones = async () => {
    const url = `/inces/secciones_filtrado`;
...
async function filterOptionsSecciones(searchTerm = '') {
    ...
    const filteredSecciones = arraySecciones.filter(sec => 
        ...
        sec.curso.toLowerCase().includes(searchTerm.toLowerCase())
    );
...
function renderOptionsSecciones(secciones) {
    customSelectSecciones.innerHTML = '';

    if (secciones.length === 0) {
        const noResults = document.createElement('div');
        noResults.className = 'no-results';
        noResults.textContent = 'No se encontraron resultados';
        customSelectSecciones.appendChild(noResults);
        return;
    }
    ...
    secciones.forEach(sec => {
        ...
        option.innerHTML = `
            <strong>${sec.curso} - ${sec.seccion}</strong>
        `;
...
function selectOptionSecciones(optionElement, seccion) {
    ...
    if (selectSec) {
        selectSec.classList.remove('selected');
    }

    // Marcar como seleccionado
    optionElement.classList.add('selected');
    selectSec = optionElement;
